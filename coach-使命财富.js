...
const COACH_MCKINSEY_CONFIG = {
  ...
  dialogTree: {
    define: [
      {
        coach: '嗨！我是你的使命财富教练。用麦肯锡结构化思维，帮你拆解使命/目标难题、找到真正有意义的人生方向。\n\n请先描述你最近遇到的一个具体使命/目标难题（尽量说清楚背景、你的期待、卡在哪里）：',
    ...
    analyze: [
      {
        coach: '明白。现在请描述一下：在这个使命/目标场景中，你具体在做什么、内心感受如何？\n\n（尽量客观描述行为和感受，不要带评判）',
        keywords: { 'default': 'analyze1' }
      },
      {
        coach: '收到。现在让我用MECE框架拆解一下这个问题：\n\n【你的行动模式】{用户选择}\n【内心/使命感反馈】待补充\n【结构因素】外部环境/资源限制/时间分配\n\n请选择你想先突破的维度：\nA. 改变我的行动模式（我能控制的部分）\nB. 改善内心/使命感反馈（建立新的意义感受）\nC. 调整结构因素（改变环境/流程/支持系统）',
        buttons: [
          { label: 'A. 改变我的行动', value: 'A', style: 'yes' },
          { label: 'B. 改善内心反馈', value: 'B', style: '' },
...
function buildAck(step, subStep, userText) {
  ...
  if (step === 'analyze') {
    if (subStep === 0) {
      ...
      return '好的，我记录了你的行动模式："' + displayText2 + '"';
    }
    if (subStep === 1) {
      return '很好！现在我有了你的行动模式，让我用MECE框架分析：\n【你的行动模式】' + (userInputs.analyze ? userInputs.analyze[0] : '') + '\n→ 内心感受到：你可能走偏了\n【内心/使命感反馈】' + userText + '\n→ 让使命管理陷入：你说你的，内心做它的\n【管理结构】外部环境 / 资源限制 / 时间分配\n\n现在，你想先从哪个维度突破？\nA. 调整我的行动模式（改变自己）\nB. 引导内心/使命感的反馈（影响意义感受）\nC. 重构管理结构（改变互动规则）';
    }
  }
  if (step === 'act') {
...
function generateDynamicQuestions(userText) {
  ...
  if (text.match(/迷茫|空虚|没意义|不知道干嘛/)) {
    questions.push('1️⃣ 在感到迷茫/空虚时，你在做什么？内心有什么声音？');
  } else if (text.match(/目标|梦想|使命|意义/)) {
    questions.push('1️⃣ 在追寻目标/使命时，你做了什么？内心感受如何？');
  } else if (text.match(/工作|职业|事业/)) {
    questions.push('1️⃣ 在工作/事业上，你做了什么？内心感受如何？');
  } else {
    questions.push('1️⃣ 当时你在做什么？内心有什么感受？');
  }

  questions.push('2️⃣ 这个问题对你的使命/意义感有什么具体影响？');
  questions.push('3️⃣ 你最希望的改变是什么？如果问题解决，你的人生会有什么不同？');
...
function replacePlaceholders(text, userText) {
  ...
  text = text.replace(/\{场景\}/g, userInputs.define ? (userInputs.define[0] ? userInputs.define[0].substring(0, 30) : '你的使命场景') : '你的使命场景');
  text = text.replace(/\{情绪\}/g, '不舒服');
  text = text.replace(/\{核心冲突\}/g, '内心期望与现实的差距');
  return text;
}
...
function generateMcKinseyReport() {
  ...
  var coreText = inputs.define ? (inputs.define[0] ? inputs.define[0].substring(0, 50) : '最近的使命场景') : '最近的使命场景';
  var behaviorText = inputs.analyze ? (inputs.analyze[0] ? inputs.analyze[0].substring(0, 30) : '某种行动模式') : '某种行动模式';

  var html = '<div class="report-header">'
    + '<div class="report-icon">📊</div>'
    + '<div class="report-title">麦肯锡风格使命财富诊断报告</div>'
    + '<div class="report-subtitle">基于结构化分析框架 · MECE原则 · 行动导向</div>'
    + '</div>'

    + '<div class="report-core-conclusion">'
    + '<h3>🎯 核心结论（金字塔顶端）</h3>'
    + '<p>【核心结论】你在"' + coreText + '"中，倾向于' + behaviorText + '，导致使命管理陷入僵局。根本原因在于行动模式形成了负向循环。</p>'
    + '</div>'

    + '<div class="report-mece-analysis">'
    + '<h3>📋 MECE结构化分析</h3>'
    + '<div class="mece-dimension"><div class="mece-dim-title">你的行动模式 <span style="color:#42a5f5">6/10</span></div><div class="mece-dim-desc">' + (inputs.analyze ? inputs.analyze[0] : '待补充') + '</div></div>'
    + '<div class="mece-dimension"><div class="mece-dim-title">内心/使命感反馈 <span style="color:#0891b2">5/10</span></div><div class="mece-dim-desc">' + (inputs.analyze ? inputs.analyze[1] : '待补充') + '</div></div>'
    + '<div class="mece-dimension"><div class="mece-dim-title">使命管理结构 <span style="color:#7c3aed">4/10</span></div><div class="mece-dim-desc">外部环境 / 资源限制 / 时间分配</div></div>'
    + '</div>'
    ...
    + '<p>' + (inputs.analyze && inputs.analyze[0] ? '【本周试验】调整你的行动方式，先倾听内心的声音' : '【本周试验】主动记录内心反应，不只在迷茫时才关注') + '<br>【判断标准】内心回应变得更清晰（知道什么让你有意义）<br>【复盘时间】建议下周一找我复盘</p>'
    + '</div>'
...
function buildDynamicActions(inputs) {
  ...
  if (allText.match(/迷茫|空虚|没意义/)) {
    actions.push({
      priority: 'P0 · 快赢',
      action: '明天早上写下一个让你感到有意义的小事（具体到时间地点）',
      impact: '建立意义感意识，打破空虚循环',
      effort: '低'
    });
    p0Added = true;
  }

  if (!p0Added && allText.match(/目标|梦想|使命/)) {
    actions.push({
      priority: 'P0 · 快赢',
      action: '今天把使命宣言写在显眼地方（手机壁纸/镜子贴）',
      impact: '用视觉提醒强化使命意识',
      effort: '低'
    });
    p0Added = true;
  ...
  if (!p0Added) {
    actions.push({
      priority: 'P0 · 快赢',
      action: '今晚睡前把明天的一件有意义行动写下来（具体到时间地点）',
      impact: '用执行意图提高意义行动概率',
      effort: '低'
    });
  }
  ...
  if (allText.match(/迷茫|空虚|没意义/)) {
    actions.push({
      priority: 'P1 · 重要',
      action: '连续7天记录"今天最有意义的时刻"（用拍照或笔记），分析什么让你有意义',
      impact: '找到意义感的关键场景，针对性设计',
      effort: '中'
    });
  } else {
    actions.push({
      priority: 'P1 · 重要',
      action: '做一次使命盘点：价值/热情/优势/影响，列出"我的使命是什么"',
      impact: '明确使命现状，指导未来行动',
      effort: '中'
    });
  }
  ...
  actions.push({
    priority: 'P2 · 长期',
    action: '读《活出生命的意义》或《使命感和你》，做笔记记录"我的使命触动点在哪里"',
    impact: '从迷茫模式切换到意义模式',
    effort: '高'
  });
